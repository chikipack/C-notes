...
int main(int argc, char **argv){
	...
	if(recv(conexion_cliente, buffer, 100, 0) < 0)//aqui estamos recibiendo el mensaje y guardandolo en el buffer
	...
	{
		printf("%s\n", buffer);//el mensaje que recibimos en la linea 42 lo imprimimos
		bzero((char *)&buffer, sizeof(buffer));//vaciamos el buffer, es decir, borramos el mensaje que nos llego porque ya lo imprimimos
		fgets(buffer, 100,stdin);
		send(conexion_cliente,buffer, 100,0);//enviamos un mensaje al cliente... creo que aqui es donde debemos modificar
	}
	close(conexion_servidor);
	return 0;
}