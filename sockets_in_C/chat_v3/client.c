...
int main(int argc, char **argv){
	...
	fgets(buffer, 100, stdin);//aqui se guarda en el buffer el mensaje escrito en consola
	send(conexion, buffer, 100, 0); //aqui se envia el mensaje que esta en el buffer
	bzero(buffer, 100);//vaciamos el buffer
	recv(conexion, buffer, 100, 0); //recepción del mensaje del servidor y lo guardamos en el buffer
	printf("%s", buffer);//con el mensaje guardado en el buffer lo imprimimos
return 0;
}